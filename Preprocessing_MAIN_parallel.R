...
#################################################
######   set up parallel preprocessing    #######
#################################################

cl <- makeCluster(detectCores()-1) # use all but one core
registerDoParallel(cl) # register workers


foreach::foreach(i = 1:6 , .packages = c("ObSortPreprocessing", "data.table")) %dopar% {

  #i <-ifelse(i == 1, 2, 3)
#for(i in 1:3){

  #source("/Users/leah/Dropbox/ObjectSorting_LTM/Distractor_task/ObSortPreprocessing/R/QuatDistance.R")

  # oxford path
  source("D:/Dropbox/Oxford/research/VirtuallySorted_LTM/ObjectSorting_LTM/Distractor_task/ObSortPreprocessing/R/QuatDistance.R")
  ...
  # oxford path
  path = "D:/Dropbox/Oxford/research/VirtuallySorted_LTM/ObjectSorting_LTM/Distractor_task/data/MAIN/"

  # read in files --> get function from preprocessing file
  data <- readData(path = paste0(path, "raw/T1/part", i), memory = F) #
  #data <- readData(path = paste0(path, "develop"), memory = F) #
  ...
  # --- 00: add dispCounter that spans both sessions

  data$dispCounter <- data$display
  data[session ==2]$dispCounter <- data[session ==2]$dispCounter + 56
  ...
  if(i == 1){
    ...
    for(n in 1:2){ # remap display counter to make consistent with other subjects
      df_main[sub == n & timepoint == 1]$display <- plyr::mapvalues(df_main[sub == n & timepoint == 1]$display, # take display counters
                                                                    from = unique(df_main[sub == n & timepoint == 1]$display), # old values
                                                                    to = c(1:56)) # map to new values
    }
  }


  # ---> E: add frameGlobal variable for easier accessing of rows
  df_main[, frameGlobal := c(1:nrow(df_main))]

  # ---> F: compute headRotations (distance between two frames)
  # prepare for head rotations (shift columns up one for easier indexing) + fill up last row with NA
  #df_main$head_yaw2 <- c(df_main$head_yaw[-(seq(1))], rep(NA, 1))
  #df_main$head_pitch2 <- c(df_main$head_pitch[-(seq(1))], rep(NA, 1))
  #df_main$head_roll2 <- c(df_main$head_roll[-(seq(1))], rep(NA, 1))
  #df_main$head_quat_w2 <- c(df_main$head_quat_w[-(seq(1))], rep(NA, 1))

  # ----- HEAD ROTATIONS ---- #
  #df_main[, headRotation := QuatDistance(frameGlobal, df_main), by = frameGlobal] # quite slow and takes a while...



  df_main$head_x2 <- c(df_main$head_x[-(seq(1))], rep(0, 1))
  df_main$head_y2 <- c(df_main$head_y[-(seq(1))], rep(0, 1))




  df_main[, headRotationYaw := angle(frameGlobal, df_main), by = frameGlobal]



  save(df_main, file =  paste0(path, "raw/T1/raw_combined_part", i, "_headRotation2.Rda"))
